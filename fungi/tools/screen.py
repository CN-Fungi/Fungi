...
SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "it lists the desktop icons, and `double_click` on one starts that application or "
            "opens that file in whatever the system opens it with — the way to open something "
            "that is running nowhere at all (no window, no taskbar button, "
            "no tray icon, so no hwnd to restore). Do not go looking for the exe on disk instead, "
            "and do not open it with a shell `start`: the icons are the app's own entry point. "
            "When the task is about the desktop, show the "
            ...
            "At hand on screen comes first, and it outranks the path you know: opening a "
            "file — its icon on the desktop, its row in a window that is already showing "
            "— is a double-click here, not a shell command. Creating, deleting, renaming "
            "or moving a file or directory is the opposite case and never comes through "
            "here: that goes through the shell — bash (mkdir / ren / move / del) or the "
            "file tools — not through screen gestures (select the row, F2, type, Enter; "
            "drag and drop). A pixel path has to be resolved into a rectangle first, and "
            "one resolved onto the wrong row is a data accident rather than a stray "
            "click. Note that del does not use the Recycle Bin (measured: the bin's item "
            "count does not move) — for anything of the user's, delete it the way "
            "Explorer does, or ask first. Drive pixels where there is no programmatic "
            "path at all. "
