...
## 37. 增补（2026-09-14 用户点名）：逐字输入，`type` 的第二种落字方式
...
原子、快，而且实测是自绘界面（微信/QQ，输入框在 a11y 里不可寻址）唯一可靠的路。
...
即"字是一个一个出现的"，不是一坨。**为什么默认还是 paste**：微信/QQ 的输入框是自绘的、a11y 里不可寻址，
...
## 39. 与「触手可及」相对的规则：文件与目录的增删改走 shell，不走像素

用户裁决（2026-09-14）：「并入与『触手可及』相对的规则：如果是对文件或目录的创建，删除或重命名，推荐采用 shell 方法」。

- **为什么**（和 §35.12/§35.15 同一条道理，只是换到文件这一侧）：窗口那一侧，应用的"门口"是任务栏/托盘/桌面图标，点它就是应用自己的打开路径；
  文件这一侧，应用的"门口"是**它自己的命令行与 API**——`mkdir` / `ren` / `move` / `del`。像素路径的每一步都要先把画面解析成矩形，
  **解析到旁边那一行就是数据事故**（§35.8 那次点击落到桌面文件上，就是这条规则的来源）。而 shell 那条路没有"解析"这一步：
  名字就是名字。
- **怎么做**：创建 / 改名 / 移动 / 删除 → `bash`（cmd）或文件工具；**不要**走「选中那一行 → F2 → 输入 → 回车」或拖拽。
  屏幕工具只在**根本没有程序路径**时才动像素（例如某个自绘列表里既没有路径也没有 API）。
- **§35.1 里「重命名用 F2」那条被部分取代**：工具面仍然接受 `key ["f2"]`（窗口内的键该发还是发），但**推荐路径变了**——
  能说清路径就别用像素。这条已同步进 `screen` 工具描述（模型面），不只写在文档里。
- **一个如实标注的反面（本机实测，2026-09-14）**：`del` / `Remove-Item` **不进回收站**。测法：看 shell 回收站命名空间的条目数。
  `cmd /c del` 一个自己建的临时文件 → 文件没了、**条目数不变（11 → 11）**；而
  ...
  → 条目数 **11 → 12**（退出码 0，目录用 `DeleteDirectory`）。所以：**动用户的东西要带走回收站的那条**，拿不准就先问人——
  这条与 §35.2「开关即许可，但护栏留在程序里」一脉相承：许可覆盖了"可以动手"，没覆盖"可以不留下痕迹"。
