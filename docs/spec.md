...
### 35.2 授权：默认关；只读与注入分开；动手要过一次确认且有时限

- **默认关**：`config.json` 的 `pc_control`（默认 `false`）。关 = 工具**不进**本机 Agent 的工具面——
  模型连看都看不到这个工具，不是"调用了才拒绝"。开 = 用户在本机设置页显式打开（设置页写明：开了之后
  **屏幕画面会随工具结果发给模型**）。
- **分两级**：打开 `pc_control` 后 **只读动作**（`shot` / `windows` / `targets`）立即可用；
  **注入动作**（`click` / `type` / `key` / `scroll`）第一次调用时弹一张确认卡（复用 ask 卡片：允许 / 不允许），
  允许后得到一段 **10 分钟**的武装期；卡片与结果里都回显剩余时间。
- **武装期到点、退房、进程退出都自动 `disarm()`**，而 `disarm()` **必须 `release_all_keys()`**——
  防"主机 Ctrl 卡死"这类只有人才能恢复的故障。
- **不可逆动作单独确认**：带修饰键的 `key`（Ctrl/Alt/Win 组合，键盘是全局的，破坏半径不由程序限定）与
  `type` 写入**本武装期内尚未确认过**的窗口，每次都要一次确认，不吃武装期。其余注入动作由武装期覆盖。
- UAC 安全桌面与提权窗口注入不进（**UIPI**）——这是硬边界，也是天然的人机分界：凡是要提权的动作，
  必须用户自己点。
...
- **发出去的那一下要再问一次**：`type` 之后在**同一窗口**按 Enter（提交/发送）属于不可逆动作，单独确认
  （`_session.typed_windows` 记住"这个窗口刚被粘过字"）；Tab 不算提交，不打扰。
